...
use crate::commitments::primitives::{Mds, Spec};
...
impl Spec<Fp, 3, 2> for P128Pow5T3 {
  ...
  fn constants() -> (Vec<[Fp; 3]>, Mds<Fp, 3>, Mds<Fp, 3>) {
    (fp::ROUND_CONSTANTS[..].to_vec(), fp::MDS, fp::MDS_INV)
  }
}
...
impl Spec<Fq, 3, 2> for P128Pow5T3 {
  ...
  fn constants() -> (Vec<[Fq; 3]>, Mds<Fq, 3>, Mds<Fq, 3>) {
    (fq::ROUND_CONSTANTS[..].to_vec(), fq::MDS, fq::MDS_INV)
  }
}
...
mod tests {
  use ff::PrimeField;
  use halo2_proofs::halo2curves::{self, group::ff};
  use std::marker::PhantomData;

  use halo2curves::FieldExt;

  use crate::commitments::{
    poseidon::{fp, fq, test_vectors},
    primitives,
  };

  use super::{Fp, Fq};
  use primitives::{permute, ConstantLength, Hash, Spec};
  ...
  fn permute_test_vectors() {
...

    {
      ...
      for tv in test_vectors::fq::permute() {
        let state = [
  ...
  fn hash_test_vectors() {
    ...
    for tv in test_vectors::fq::hash() {
      let message = [
