...
use crate::commitments::primitives::Mds;

use super::grain::Grain;

pub fn generate_mds<F: FieldExt, const T: usize>(
  grain: &mut Grain<F>,
  mut select: usize,
) -> (Mds<F, T>, Mds<F, T>) {
  let (xs, ys, mds) = loop {
...
mod tests {
  ...
  use crate::commitments::poseidon::{self, grain::Grain};
  ...
  fn poseidon_mds() {
    const T: usize = 3;
    let mut grain = Grain::new(poseidon::grain::SboxType::Pow, T as u16, 8, 56);
