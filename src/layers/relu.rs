...
#[derive(Clone, Debug)]
pub struct ReluLayerChip {}

impl<F: PrimeField> Layer<F> for ReluLayerChip {
  fn forward(
    &self,
    ...
    let ReluLayerChip = ReluChip::<F>::construct(gadget_config.clone());
    let vec_inps = vec![inp_vec];
    let constants = vec![zero];
    let out = ReluLayerChip.forward(layouter.namespace(|| "relu chip"), &vec_inps, &constants)?;
...
impl GadgetConsumer for ReluLayerChip {
  fn used_gadgets(&self, _layer_params: Vec<i64>) -> Vec<crate::gadgets::gadget::GadgetType> {
    vec![GadgetType::Relu, GadgetType::InputLookup]
  }
}
